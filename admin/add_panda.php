...
if (isset($_GET['id']) && $_GET['id'] != '') {
    ...
    $row = mysqli_fetch_assoc(mysqli_query($con, "select * from past_question where id='$id' "));
...
if (isset($_POST['submit'])) {
    ...
    if ($id == '') {
        ...
        mysqli_query($con, "insert into past_question(file,title,status,description,level,department,image) values('$new_file','$title',1,'$description','$level','$department','$image') ");
        redirect('pqanda.php');
    } else {
        $file = "e-library(forestry.com)" . '_' . $_FILES['file']['name'];
        $image = rand(111111111, 999999999) . '_' . $_FILES['image']['name'];
        ...
        move_uploaded_file($_FILES['image']['tmp_name'], FILE_SERVER_PATH . $image);
        move_uploaded_file($_FILES['file']['tmp_name'], FILE_SERVER_PATH . $new_file);
        mysqli_query($con, "update  past_question set title='$title',description='$description',level='$level',department='$department',image='$image' where id='$id'");
        redirect('bb..bb.php');
    }
}
