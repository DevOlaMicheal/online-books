...
if (isset($_GET['id']) && $_GET['id'] != '') {
    ...
    $row = mysqli_fetch_assoc(mysqli_query($con, "select * from uploaded_videos where id='$id' "));
...
if (isset($_POST['submit'])) {
    ...
    if ($id == '') {
        ...
        move_uploaded_file($_FILES['file']['tmp_name'], FILE_SERVER_PATH . $file);
        mysqli_query($con, "insert into uploaded_videos(file,title,status,description,level,department,image,type) values('$file','$title',1,'$description','$level','$department','$image','$type') ");
        redirect('video_tuts.php');
    } else {
        $file = "e-library(forestry.com)" . '_' . $_FILES['file']['name'];
        $image = rand(111111111, 999999999) . '_' . $_FILES['image']['name'];
        //prx($_POST);
        move_uploaded_file($_FILES['image']['tmp_name'], FILE_SERVER_PATH . $image);
        move_uploaded_file($_FILES['file']['tmp_name'], FILE_SERVER_PATH . $file);
        mysqli_query($con, "update  uploaded_videos set title='$title',description='$description',level='$level',department='$department',image='$image',type='$type' where id='$id'");
        redirect('bb..bb.php');
    }
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-danger">
                        ...
                        <div class="form-group">
                                    <select class="custom-select form-control-border border-width-2" id="exampleSelectBorderWidth2" name="type">
